...
class Task:
    ...
    def init_task(self):
        ...
        self.resume_model()                                                                             # 重新加载模型，也就是在训练20轮的情况下继续训练
    ...
    def resume_model(self):
        if self.params.resume_model:                                                                                    # 要加载训练过的模型
            logger.info(f'Resuming training from {self.params.resume_model}')
            loaded_params = torch.load(f"saved_models/{self.params.resume_model}", map_location=torch.device('cpu'))    # 加载模型配置项
            self.model.load_state_dict(loaded_params['state_dict'])                                                     # 加载模型参数
            self.params.start_epoch = loaded_params['epoch']                                                            # 加载轮次
            self.params.lr = loaded_params.get('lr', self.params.lr)                                                    # 加载学习率
            logger.warning(f"Loaded parameters from saved model: LR is {self.params.lr} and current epoch is {self.params.start_epoch}")
    ...
    def report_metrics(self, step, prefix='', tb_writer=None, tb_prefix='Metric/'):
        ...
        logger.warning(f'{prefix} {step:4d}. {" | ".join(metric_text)}')                                             # Backdoor {str(backdoor):5s}. Epoch:{step:3d}.
