...
class FederatedLearningTask(Task):
    ...
    def init_task(self):
        ...
        self.adversaries = self.sample_adversaries()                                          # 直接用entrophy了，
        self.metrics = [AccuracyMetric(), TestLossMetric(self.criterion)]                                    # 这个是选择攻击的客户端
    ...
    def sample_users_for_round(self, epoch) -> List[FLUser]:
        ...
        for pos, user_id in enumerate(sampled_ids):                                                                     # 对于每个随机选中的client
            ...
            compromised = self.check_user_compromised(epoch, pos, user_id)                                              # 设置攻击者
    ...
    def check_user_compromised(self, epoch, pos, user_id):
        ...
        """
        compromised = False
        if self.params.fl_single_epoch_attack is not None:                                                              # 是单轮攻击，就需要设置fl_number_of_adversaries且小于10
    ...
    def sample_adversaries(self) -> List[int]:
        ...
        elif self.params.fl_single_epoch_attack is None:                        # 不是单轮攻击
            adversaries_ids = random.sample(range(self.params.fl_total_participants), self.params.fl_number_of_adversaries)
            logger.warning(f'Attacking over multiple epochs with following users compromised: {adversaries_ids}.')
        else:
            logger.warning(f'Attack only on epoch: '                        # 单轮攻击
