...
class CifarFedTask(FederatedLearningTask, Cifar10Task):
    ...
    def load_data(self) -> None:
        print("cifarfed_task→下载ciar10数据")
        self.load_cifar_data()                                                                                          # 下载数据集并进行加载cifar10_task.py
        print("平分数据")
        if self.params.fl_sample_dirichlet:
            # 使用狄利克雷分布来进行用户训练数据的取样，这段代码的目的是为联邦学习（Federated Learning）设置参与者的训练数据。
            # sample indices for participants using Dirichlet distribution
            indices_per_participant = self.sample_dirichlet_train_data(self.params.fl_total_participants, alpha=self.params.fl_dirichlet_alpha)
            ...
            all_range = list(range(len(self.train_dataset)))                                                            # 训练集图片的索引，0-49999
            print("打乱训练集")
            random.shuffle(all_range)                                                                                   # 打乱训练数据
            print("给每个客户端加载数据")
            train_loaders = [self.get_train_old(all_range, pos) for pos in range(self.params.fl_total_participants)]    # pos=0-99
    ...
    def get_train(self, indices):
        ...
        :return:
        """
        train_loader = DataLoader(self.train_dataset, batch_size=self.params.batch_size, sampler=SubsetRandomSampler(indices))
        return train_loader
