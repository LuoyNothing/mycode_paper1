...
class Attack:
    ...
    def compute_blind_loss(self, model, criterion, batch, attack):
        ...
        :return:
        """
        batch = batch.clip(self.params.clip_batch)                                                      # 将batch的值限制在clip_batch范围内
        loss_tasks = self.params.loss_tasks.copy() if attack else ['normal']                            # 如果是攻击，就loss_tasks = 同时训练后门任务和正常任务
        ...
        if self.params.loss_threshold and (np.mean(self.loss_hist) >= self.params.loss_threshold or len(self.loss_hist) < 1000):
            loss_tasks = ['normal']
